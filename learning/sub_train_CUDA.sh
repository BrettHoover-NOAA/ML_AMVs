...
nEpochs=1
annealing=0.25
modelName=SupErrNet_v2.2.1_CUDA
...
jid_train=$(sbatch --parsable -A da-cpu -N 1 -t ${runTime} -p fge -q gpuwf --job-name=${jName_train} run_train_SupErrNet-CUDA.sh ${trainDir} ${statsFile} ${epoch} ${annealing} ${runDir} ${modelName})
####################################################################################
...
do
    ...
    jid_train=$(sbatch --parsable -A da-cpu -N 1 -t ${runTime} -p fge -q gpuwf --dependency=afterany:${jid_prior} --job-name=${jName_train} run_train_SupErrNet-CUDA.sh ${trainDir} ${statsFile} ${epoch} ${annealing} ${runDir} ${modelName})
    # increment epoch
    ((epoch++))
