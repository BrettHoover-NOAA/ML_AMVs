...
def validate_epoch(model, loss_fn, dataValidLoader, labelValidLoader, cnnValidLoader, statsTuple):
    #
    ...
    with torch.no_grad():
        for i in range(len(dataValidLoader)):
            ...
            # further filter out any index where vC.sum(axis=1)=NaN. (any NaN values on subgrid)
