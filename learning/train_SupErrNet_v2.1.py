...
class SupErrNetv2(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
                                nn.Linear(16*6*6+36,409),  # input layer
                                nn.LeakyReLU(),
                                nn.Linear(409, 274),  # hidden layer 1
                                nn.LeakyReLU(),
                                nn.Linear(274, 184),  # hidden layer 2
                                nn.LeakyReLU(),
                                nn.Linear(184, 124),  # hidden layer 3
                                nn.LeakyReLU(),
                                nn.Linear(124, 84),  # hidden layer 4
                                nn.LeakyReLU(),
                                nn.Linear(84, 57),  # hidden layer 5
                                nn.LeakyReLU(),
                                nn.Linear(57, 1)    # output layer
                               )
...
def train_and_validate_epoch(model, loss_fn, optimizer, dataTrainLoader, labelTrainLoader, cnnTrainLoader, dataValidLoader, labelValidLoader, cnnValidLoader, statsTuple, miniBatchSize):
    ...
    for i in range(len(dataTrainLoader)):
        ...
        for k in range(nMiniBatches):
            ...
            outputs = model(C[kpBeg:kpEnd,:,:].float().unsqueeze(dim=1), X[:,kpBeg:kpEnd].T.float())  # X is transposed and asserted as torch.float
            # compute the loss
            if len(Y.shape) == 1:
        ...
        #t0 = time.time()
        #with torch.no_grad():
        #    outputs = model(C.float().unsqueeze(dim=1), X.T.float())
        ...
        #t1 = time.time()
        #print('retrained whole batch in {:.2f} seconds'.format(t1-t0))
    #
    # validate model
    #
    ...
    with torch.no_grad():
        ...
        for i in range(0): #range(len(dataValidLoader)):
            ...
            kp2 = torch.where(np.isnan(vC.sum(axis=1))==False)[0].numpy()
            kp = np.intersect1d(kp1, kp2)
            vX = vX[:,kp]
            if len(Y.shape) == 1:
                vY = vY[kp]
                ...
                vY = vY[:,kp]
            # make predictions for this batch
            vout = model(vC.float().unsqueeze(dim=1), vX.T.float())
            # compute loss
            if len(vY.shape) == 1:
