...
def train_epoch(model, loss_fn, optimizer, dataTrainLoader, labelTrainLoader, statsTuple, miniBatchSize):
    ...
    for i in range(len(dataTrainLoader)):
        ...
        for k in range(nMiniBatches):
            ...
            outputs = model(X[:,kpBeg:kpEnd].T.to(torch.float32))  # X is transposed and asserted as torch.float
            # compute the loss
            if len(Y.shape) == 1:
                loss = loss_fn(outputs.T, Y[None,kpBeg:kpEnd].to(torch.float32))  # use [None,:] to force dim=0 as a singleton dimension if Y is a vector
            else:
                loss = loss_fn(outputs.T, Y[:,kpBeg:kpEnd].to(torch.float32))
            optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    lossFunc = torch.nn.MSELoss()  # v1.0
    lossFunc = torch.nn.HuberLoss(delta=10.)  # v1.0.1
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=learningRate)  # v.1.0
    optimizer = torch.optim.AdamW(model.parameters(), lr=learningRate, weight_decay=1.0E-02)  # v.1.0.1
