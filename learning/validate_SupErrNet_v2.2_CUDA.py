    ...
    with torch.no_grad():
        for i in range(len(dataValidLoader)):
            ...
            if len(vY.shape) == 1:
                ...
                vY = vY[:,kp]
            vC = cnnVal[kp,:,:]
            # pass batch data to device
            vC = vC.to(device)
            vX = vX.to(device)
            vY = vY.to(device)
            # make predictions for this batch
            vout = model(vC.to(torch.float32).unsqueeze(dim=1), vX.T.to(torch.float32))
            # compute loss
            if len(vY.shape) == 1:
                vloss = loss_fn(vout.T, vY[None,:].to(torch.float32))
            else:
                vloss = loss_fn(vout.T, vY.to(torch.float32))
